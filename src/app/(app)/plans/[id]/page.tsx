...
const TABS: { key: string; label: string; icon: typeof Compass; resource: PermissionResource | null }[] = [
  { key: "overview", label: "نظرة عامة", icon: Compass, resource: null },
  { key: "objectives", label: "الأهداف", icon: Target, resource: "objectives" },
  { key: "campaigns", label: "الحملات", icon: Rocket, resource: "campaigns" },
  { key: "content", label: "المحتوى", icon: FileText, resource: "content" },
  { key: "budget", label: "الميزانية", icon: Wallet, resource: "budget" },
  { key: "tasks", label: "المهام", icon: ListChecks, resource: "tasks" },
];
...
export default async function PlanDetailPage({
  ...
  if (!can(user, "plans", "VIEW")) return <AccessDenied label="الخطة التسويقية" />;
  const canEditPlan = can(user, "plans", "EDIT");
  const canEditObjectives = can(user, "objectives", "EDIT");
  const canEditCampaigns = can(user, "campaigns", "EDIT");
  const canEditContent = can(user, "content", "EDIT");
  const canEditBudget = can(user, "budget", "EDIT");
  ...
  // Tabs surface data from other resources — only show/serve the ones this user can view,
  // and never trust the ?tab= query param for a resource they can't see.
  const visibleTabs = TABS.filter((t) => !t.resource || can(user, t.resource, "VIEW"));
  const tab = visibleTabs.some((t) => t.key === requestedTab) ? requestedTab : "overview";
                ...
                <Tbody>
                  {plan.tasks.map((t) => (
                    <Tr key={t.id}>
                      ...
                      <Td>{canEditTasks && <DeleteButton action={deleteTask.bind(null, t.id)} />}</Td>
                    </Tr>
                  ))}
                </Tbody>
              </Table>
